...
app.get('/home/team_stats', function(req, res){
	var quer1 = 'SELECT * FROM football_games';
	var quer2 = 'SELECT count(*) FROM football_games WHERE home_score > visitor_score';
	var quer3 = 'SELECT count(*) FROM football_games WHERE visitor_score > home_score';
...
app.get('/player_info/post', function(req,res){
	var ply_id = req.query.player_choice;

	console.log(ply_id);

	var mainInfo = "SELECT * FROM football_players WHERE id='" + ply_id + "';";
	var allPlayers = 'SELECT id, name FROM football_players;';
	var gPlayed = "SELECT count(*) FROM football_games WHERE '" + ply_id + "'=ANY(players);"
	db.task('get-everything', task => {
        return task.batch([
            task.any(mainInfo),
			task.any(allPlayers),
...
app.get('/home/player_info', function(req,res){
	var quer = 'SELECT id, name FROM football_players;';
	db.any(quer)
		.then(function(fbplys) {
			res.render('pages/player_info',{
				my_title: "Player Info",
				nData: fbplys,
			})
